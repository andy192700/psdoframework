...
/// <summary>
/// Responsible for creating and registering a process.
/// </summary>
...
public class ProcessCreator(
    ...
    ILogger logger) : IDataCreator<ProcessDescriptor>
{
    ...
    /// <summary>
    /// Creates a new process and registers it.
    /// </summary>
    /// <param name="item">The process descriptor to create and register.</param>
    public void Create(ProcessDescriptor item)
    {
        var contents = _readProjectContents.Provide();

        contents.Processes.Add(item);

        _saveProjectContents.Create(contents);

        _logger.LogInfo($"Process registered at {item.Name} with path {item.Path}");
    }
}
