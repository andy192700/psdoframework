...
public class TestDeletorTests
{
    [Theory]
    [InlineAutoMoqData]
    public void TestDeletor_CannotDeleteTest(
        string name,
        string path)
    {
        // Arrange
        ...
        logger.Verify(x => x.LogWarning($"Test: {descriptor.Name} could not be located for removal."), Times.Once);
    }

    [Theory]
    [InlineAutoMoqData]
    public void TestDeletor_DeletesTest(
        string name,
        string path)
    {
        // Arrange
        ...
        logger.Verify(x => x.LogInfo($"Test: {descriptor.Name} removed."), Times.Once);
